...
export async function POST(req) {
  ...
  const { data: last } = await sb.from("players").select("rank").order("rank", { ascending: false }).limit(1).single();
  const nextRank = (last?.rank || 0) + 1;

  const { data: player, error } = await sb
    .from("players")
    .insert({ name: name.trim(), emoji: emoji || "⚽", rank: nextRank })
    .select()
    .single();

  if (error) return NextResponse.json({ error: error.message }, { status: 500 });

  await logAudit("add_player", `Added ${player.name} at rank ${nextRank}`);
